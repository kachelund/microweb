...
class BiolinkBlockAjax extends Controller {
    ...
    private function create() {
        ...
        if(isset($_POST['type']) && array_key_exists($_POST['type'], require APP_PATH . 'includes/biolink_blocks.php')) {
    ...
    private function handle_image_upload($uploaded_image, $upload_folder, $size_limit) {
        ...
        if($image) {
            ...
            if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                try {
                    ...
                    if(!empty($uploaded_image)) {
                        $s3->deleteObject([
                            'Bucket' => settings()->offload->storage_name,
                            'Key' => UPLOADS_URL_PATH . $upload_folder . $uploaded_image,
                        ]);
                    }
            ...
            /* Local uploading */
                ...
                /* Delete current file */
                if(!empty($uploaded_image) && file_exists(UPLOADS_URL_PATH . $upload_folder . $uploaded_image)) {
                    unlink(UPLOADS_URL_PATH . $upload_folder . $uploaded_image);
                }

                /* Upload the original */
                move_uploaded_file($image_file_temp, UPLOADS_URL_PATH . $upload_folder . $image_new_name);
            }
